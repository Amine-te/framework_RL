...
from scripts.experiments.q_learning_moving_goal import PositionOnlyAgent
...
class LinearFAAgent:
    ...
    def get_features(self, agent_pos):
        ...
        # Observations may be numpy arrays with float dtype; ensure integer indices
        row, col = agent_pos
        row = int(row)
        col = int(col)
...
def run_comparison(episodes=5000, grid_size=7):
    ...
    filename = f'func_approx_no_goal_ep{episodes}_grid{grid_size}_comparison.png'
